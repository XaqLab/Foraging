...
from foraging.utils.data import bin_data, filter_df, process_block_safely, process_blocks
...
from foraging.plotting import PALETTE, HEATMAP_PALETTE, bp, fig_init, titler, legend_handler
from scipy.stats import gamma
from scipy.optimize import minimize_scalar
from functools import reduce


def likelihood_single_obs(obs_model: Observation, latents: ArrayLike, obs: Any, ax: Optional[plt.Axes] = None,
                          **kwargs: Any) -> plt.Axes:
    """
...
@legend_handler
def plot_fisher_info(t: Any, schedules: list[float], palette: dict = PALETTE, alpha: float = 1, figsize: tuple[float, float] = (10, 5), title: str = "Fisher Information"):
    ...
    for i, schedule in enumerate(schedules):
        ...
        print(f"Optimal Fisher info for schedule {schedule} with shape = {alpha} occurs at t = {x_max:.4f}, with value {f_max:.4f}")
    ...
    ax.vlines(x = optimal_fisher_t.values(), ymin = 0, ymax = 1.1 * ymax, color = colors, linestyle = '--', label = r'$t^*=\underset{t}{\mathrm{argmax}}J_{\lambda}(t)$')
    ax.set_title(title)
    
    # Create a black dashed line for the legend
    optimal_line = Line2D([0], [1], color='black', linestyle='--', label='optimal fisher info')
...
@legend_handler
def plot_optimal_fisher_uncertainty(n: int, schedules: list[float], alpha: float = 1, title: str = "Fisher-optimal uncertainty as a function of number of pushes"):
    n = np.arange(n) + 1
...
def plot_schedule_beliefs_in_block(df: pd.DataFrame, beliefs: dict[tuple, BeliefModule], conds: dict[str, Any], palette: dict = PALETTE, heatmap_palette: dict = HEATMAP_PALETTE, show_stats: bool = False, ax: plt.Axes = None,
                           **kwargs) -> plt.Axes:
    """
    ...
    interpolated_beliefs = np.zeros((new_num_cols, posteriors.shape[1], posteriors.shape[2]))
    start = 0
    ...
    if show_stats:
        mean_across_time = get_mean_beliefs(interpolated_beliefs, schedule_candidates)  # E[lambda] at each timepoint
        std_across_time = np.nan_to_num(get_std_beliefs(interpolated_beliefs, schedule_candidates))
    fig, ax = fig_init(ax, **kwargs.pop('fig_kwargs', {'nrows': n_boxes, 'ncols': 1, 'sharex': True, 'sharey': True, 'figsize': (15, 10)}))
    box_labels = list(heatmap_palette.keys())
    for i in range(n_boxes):
        # Plot belief probabilities
        sns.heatmap(interpolated_beliefs[:, i, :].T, cmap = heatmap_palette[box_labels[i]], ax = ax[i])
        if show_stats:
            ax[i].plot(mean_across_time[:, i], color = palette[box_labels[i]], label = 'mean')
            ax[i].plot(mean_across_time[:, i] - std_across_time[:, i], color = palette[box_labels[i]], linestyle = ':', label = 's.d.')
            ax[i].plot(mean_across_time[:, i] + std_across_time[:, i], color = palette[box_labels[i]], linestyle = ':')
        ...
        ax[i].set_yticklabels([schedule_candidates[int(j)] for j in current_yticks if int(j) < len(schedule_candidates)])

        # Plot reward outcomes
        reward_mask = df_block['reward outcomes'].values
        box_mask = df_block['box rank'].values == i
        ax[i].scatter(push_times[reward_mask & box_mask], mean_across_pushes[reward_mask & box_mask, i], c='black', marker='^', s = 50) 
        ax[i].scatter(push_times[~reward_mask & box_mask], mean_across_pushes[~reward_mask & box_mask, i], edgecolors='black', marker='v', s = 50, facecolors = 'none') 
        
        # Add legend
        if i == 0:
            handles = [Line2D([0], [0], color='black', linestyle='--', label='true schedule'), Line2D([0], [0], color='black', linestyle='', marker='^', label='rewarded'), Line2D([0], [0], color='black', linestyle='', marker='v', markerfacecolor = 'none', label='no reward')]
            if show_stats:
                handles.append(Line2D([0], [0], color = 'black', linestyle = '-', label = 'mean'))
                handles.append(Line2D([0], [0], color = 'black', linestyle = ':', label = 's.d.'))
...
def plot_schedule_beliefs_mean_and_std_across_blocks(df: pd.DataFrame, beliefs: dict[tuple, BeliefModule], conds: dict[str, Any] = None, x: str = 'push times', palette: dict = PALETTE,
                           **kwargs) -> plt.Axes:
    """
    Plots the beliefs about the schedule for a specific block in the experiment, with uncertainty bands
    and reward outcomes.
    ...
    def _inner(df: pd.DataFrame, index: tuple):
        ...
        res = {(index + (i,)): [mean[i + 1, box_rank[i]], std[i + 1, box_rank[i]], push_times[i], box[i], i] for i in range(len(mean) - 1)}
    ...
    df_beliefs = pd.DataFrame.from_dict(res, orient = 'index', columns = ['mean of belief', 's.d. of belief', 'push times', 'box', 'n pushes'])
    ...
    def _plot(i, subj, **kwargs):
        ...
        bp(sns.lineplot)(df_subj, x = x, y = 'mean of belief', hue = 'box', palette = palette, ax = ax[0], **kwargs)
        bp(sns.lineplot)(df_subj, x = x, y = 's.d. of belief', hue = 'box', palette = palette, ax = ax[1], legend = False, **kwargs)
