...
# $$
#
# where $f$ and $F$ are the pdf and cdf of the gamma distribution, respectively, and $\alpha$ denotes the shape parameter, which we assume to be known*. From these, we can derive lower bounds on how fast the uncertainty can shrink for the ideal observer. Assuming a continuous hypothesis space over possible schedules, and independence between boxes, one way to do this is to derive the Fisher information, which we go into next section.
#
# ## Fisher Information
#
...
#
# Now, this framework is only applicable if we assume the subjects 1) treat the boxes as independent and 2) start their inference process from scratch at the start of each block. If the schedules are kept constant over several blocks and merely permuted, then it is conceivable that the subjects eventually get a good handle on the identities of the schedules and only infer the assighment of specific schedules to boxes, not the values of the schedules themselves. In that case, their beliefs are defined over $m!$ permutations, where $m$ is the number of boxes. Analogous to the Fisher information, one can calculate the average likelihood ratio (or difference of log-likelihoods) between each pair of permutations. Specifically, consider permutations $\theta$ and $\theta^{\prime}$ and observations $o_{1:n}$ generated under $\theta$. Then, one may be interested in how well the observations $o_{1:n}$ differentiate the two hypotheses by comparing $L(\theta)$ vs $L(\theta^{\prime})$ under $o_{1:n}$. This will result in a square matrix where each entry $(\theta, \theta^{\prime})$ is the likelihood ratio (or log-likelihood difference) averaged over many simulations of $o_{1:n}$.
#
# #TODO: numerically implement this matrix, possibly as a function of time
